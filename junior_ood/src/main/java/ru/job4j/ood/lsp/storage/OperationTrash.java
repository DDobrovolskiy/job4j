...
public class OperationTrash extends OperationStoreBase implements OperationStore {
    private final StoreFood storeFood;

    public OperationTrash(StoreFood storeFood) {
        this.storeFood = storeFood;
    }

    @Override
    public boolean add(Food food, ChronoLocalDate toDay) {
        if (getPercentFreshFood(food, toDay) >= 100) {
