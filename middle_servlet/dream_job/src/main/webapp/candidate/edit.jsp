    ...
    <script>
            $(document).ready(function() {
                $.ajax({
                    ...
                    for (var cityAjax of data) {
                    $('#cityId').append('<option value=' + cityAjax.id + '>' + cityAjax.name + '</option>')
                }
                }).fail(function(err){
                    alert(err);
                });
...
<%
    ...
    Candidate candidate = new Candidate(0, "", 1, LocalDateTime.now());
    if (id != null) {
            ...
            <div class="card-body">
                <form action="<%=request.getContextPath()%>/candidates.do?id=<%=candidate.getId()%>" method="post">
                    <div class="form-group">
                        <label>Имя</label>
                        <input type="text" class="form-control" name="name" value="<%=candidate.getName()%>" id="name" title="Enter name">
                    </div>
                    <div class="form-group">
                        <label>Город</label>
